...
public abstract class AdapterView<T extends Adapter> extends ViewGroup {
	...
	/**
	 * Sync based on the last child displayed
	 */
	...
	public int getPositionForView(View view) {
		...
		try {
			...
			Log.printErrStackTrace("AdapterView", e, null, null);
	...
	void selectionChanged() {
		...
		if (mSelectedPosition != INVALID_POSITION && isShown()
				&& !isInTouchMode()) {
	...
	// * @return
	// */
	// protected static int getAttrValue(String fieldName)
	// {
	// try
	// {
	 ...
	 * @return
	 */
	protected static int getStyleableValue(String fieldName) {
		try {
	...
	protected static int[] getStyleableValues(String fieldName) {
		try {
			Class clz = Class.forName("com.android.internal.R$styleable");
			Field field = clz.getField(fieldName);
	...
	protected void invalidateParentIfNeeded() {
		if (SDKVersionUtilsKt.isHoneycomb() && isHardwareAccelerated()
	...
	protected void invalidateParentCaches() {
		if (getParent() instanceof View) {
			try {
				...
				int flags = f.getInt(getParent());
				f.set(getParent(), flags | 0x80000000);

				// ((View) getParent()).mPrivateFlags |= 0x80000000;
			} catch (Exception e) {
				Log.printErrStackTrace("AdapterView", e, null, null);
	...
	protected float getVerticalScrollFactor() {
		if (mVerticalScrollFactor == 0) {
			...
			mVerticalScrollFactor = outValue.getDimension(getContext()
	...
	public static void traceBegin(String methodName) {
		TraceUtils.traceBegin(methodName);
	}
