...
import com.qq.reader.core.utils.SDKVersionUtilsKt;
import com.tencent.mars.xlog.Log;
import com.tencent.util.AnimateUtils;
import com.tencent.widget.XBaseAdapter.ListDataSetListener;
...
/**
 * 4.0向下兼容Android 2.1的ListView. 裁剪掉得功能有。。findViewByid(会搜索head和foot)
 * 调用hasOpaqueScrollbars判断透明（较后得版本才支持）. 主要的功能: 1. OverScroll功能 2. 硬件加速优化 3.
 * 滚动状态(scroll, fling, idle)回调 4. TranscriptMode, 是否要在更新时候滚动到最后一行
 * 
 * 追加的功能 1. 修改stackFromBottom逻辑, 当布局改变时保持最后一行位置不变, 使其更加符合聊天场景. 2. 添加增量更新接口
 * 添加更新动画(需要使用上面的接口, 目前已实insert和delete动画) 3. 当位于最后一行时, 有数据更新则自动向上滚动(针对AIO的场景) 4.
 * 下拉刷新功能 5. 重写Overscroll回弹算法. 使其看上去更加"正常"(旧算法是按照固定的位置进行回弹的, 即时你速度为1也会弹一个固定的长度).
 * 6. 添加边界动画(蓝色Glow效果)的开关
 * 
 * 核心方法: layoutChildren. 每次触发request layout时, 将会调用此方法对整个ListView进行重新布局.
 * 针对mStackBottom进行了修改 trackMotionScroll. 用于list的滚动 OverScroller.
 * 重写了回弹算法(edgeReached)
 * 
 * 添加参数mSpecificBottom, 用来计算当onDataSetChanged时, 距离最底部的位置
 * 
 * 添加常量OVERSCROLL_STATUS_IDLE,
 * OVERSCROLL_STATUS_VISABLE,OVERSCROLL_STATUS_COMPLETE_VISABLE
 * ,OVERSCROLL_STATUS_COMPLETE_RELEASE 用来记录下拉刷新的状态改变
 * 
 * 添加ListDataSetObserver类, 用来接收增量更新的通知
 * 
 ...
 */
@RemoteView
public class ListView extends AbsListView // 先把皮肤引擎部分去掉 implements SkinnableView
{
	@Override
	public boolean showContextMenuForChild(View originalView,float a,float b) {
		return false;
	}
	...
	/**
	 * kanedong[03/06/2014] ListView 原本的实现部分情况下 OverScrollHeadView 回弹后
	 * mOverScrollHeadState 不会置为 IDLE，例如手动将 OverScrollHeadView 收到一半后
	 * 释放回弹时。这就导致下次拉下 OverScrollHeadView 时其起始状态不正确。
	 * 该问题导致个性资料卡的新需求实现上有障碍，所在在此增加一个标志，为 true 时保证回弹后状态置 IDLE。为不影响原来的业务逻辑标志的默认值为
	 * false
	 */
	protected boolean mEnsureOverScrollStatusToIdleWhenRelease = false;
	...
	protected int mOverscrollHeadState = OVERSCROLL_STATUS_IDLE;
	...
	public ListView(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArrayWarpper a = new TypedArrayWarpper(
				context.obtainStyledAttributes(attrs,
						getStyleableValues("ListView"), defStyle, 0));

		CharSequence[] entries = a.getTextArray(LISTVIEW_ENTRIES);
		if (entries != null) {
			setAdapter(new ArrayAdapter<CharSequence>(context,
					android.R.layout.simple_list_item_1, entries));
		}

		final Drawable d = a.getDrawable(LISTVIEW_DIVIDER);
		if (d != null) {
		...
		final Drawable osHeader = a.getDrawable(LISTVIEW_OVERSCROLLHEADER);
		if (osHeader != null) {
			setOverscrollHeader(osHeader);
		}

		final Drawable osFooter = a.getDrawable(LISTVIEW_OVERSCROLLFOOTER);
		if (osFooter != null) {
		...
		final int dividerHeight = a.getDimensionPixelSize(
				LISTVIEW_DIVIDERHEIGHT, 0);
		if (dividerHeight != 0) {
			setDividerHeight(dividerHeight);
		}

		mHeaderDividersEnabled = a.getBoolean(LISTVIEW_HEADERDIVIDERSENABLED,
				true);
		mFooterDividersEnabled = a.getBoolean(LISTVIEW_FOOTERDIVIDERSENABLED,
				true);
	...
	// overscrollview高度可以设置，而并不是一定为overheaderview的高度 by august
	// 场景：mOverscrollHeaderView需要一个较大的背景，触发刷新的下拉高度小于mOverscrollHeaderView的高度
	...
	/**
	 * 获取的触发刷新的下拉高度，没设置就直接用mOverscrollHeaderView的高度
	 * 
	 * @return
	 */
	public int getOverScrollHeight() {
		if (mOverScrollHeight == 0 && mOverscrollHeaderView != null) {
			return mOverscrollHeaderView.getHeight();
		} else {
	...
	public int getOverScrollFooterHeight() {
		if (mOverscrollFooterView == null) {
			return 0;
		} else {
			return mOverscrollFooterView.getHeight();
		}
	}
	...
	public int getMaxScrollAmount() {
		return (int) (MAX_SCROLL_FACTOR * (getBottom() - mTop));
	}

	/**
	 * 当listView内item高度小于listView总高度时候, 保持从上方排序
	 */
	private void stayOnTheTop() {
		// final int childCount = getChildCount();
		if (mStackFromBottom && mFirstPosition == 0 && getChildCount() > 0) {
			// Uh-oh -- we came up short. Slide all views up to make them
			// align with the top
			View child = getChildAt(0);
			final int delta = child.getTop() - mListPadding.top;
			if (delta > 0) {
	...
	// /**
	// * Sets up this AbsListView to use a remote views adapter which connects
	// to a RemoteViewsService
	// * through the specified intent.
	// * @param intent the intent used to identify the RemoteViewsService for
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if (mAdapter != null) {
			...
			mDataSetObserver = new ListDataSetObserver();
	...
	void fillGap(boolean down) {
		...
		if (down) {
			...
			final int startOffset = count > 0 ? getChildAt(count - 1)
					.getBottom() + mDividerHeight : paddingTop;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (mItemCount > 0
			...
			if (SDKVersionUtilsKt.isHoneycomb()) {
		...
		} else if (SDKVersionUtilsKt.isHoneycomb()) {
		...
		if (mOverscrollHeaderView != null) {
			...
			mOverscrollHeaderView.measure(childWidthSpec, childHeightSpec);
		}
		if (mOverscrollFooterView != null) {
			int childWidthSpec = ViewGroup.getChildMeasureSpec(
					mWidthMeasureSpec, mListPadding.left + mListPadding.right,
			...
			int childHeightSpec = MeasureSpec.makeMeasureSpec(0,
					MeasureSpec.UNSPECIFIED);
			mOverscrollFooterView.measure(childWidthSpec, childHeightSpec);
		}
	}

	// private int getChildMeasureState(View child)
	// {
	// return (child.getMeasuredWidthAndState() & MEASURED_STATE_MASK)
	// | ((child.getMeasuredHeightAndState() >> MEASURED_HEIGHT_STATE_SHIFT) &
	// (MEASURED_STATE_MASK >>
	...
	private View fillSpecificBottom(int position, int bottom) {
		boolean tempIsSelected = position == mSelectedPosition;
		View temp = makeAndAddView(position, bottom, false, mListPadding.left,
				tempIsSelected);
	...
	/**
	 * List底部有空隙.向下滚动, 并填补顶部 Check if we have dragged the bottom of the list too
	 * high (we have pushed the top element off the top of the screen when we
	 * did not need to). Correct by sliding everything back down.
	 * 
	 * @param childCount
	 *            Number of children
	 */
	private void correctTooHigh(int childCount) {
		...
		if (lastPosition == mItemCount - 1 && childCount > 0) {
			...
			final int end = (getBottom() - mTop) - mListPadding.bottom;
	...
	/**
	 * List顶部有空隙.向上滚动, 并填补底部 Check if we have dragged the bottom of the list too
	 * low (we have pushed the bottom element off the bottom of the screen when
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mOverscrollHeaderView != null) {
			mOverscrollHeaderView.layout(
					mListPadding.left,
					mListPadding.top,
					mListPadding.left
							+ mOverscrollHeaderView.getMeasuredWidth(),
					mOverscrollHeaderView.getMeasuredHeight());
			mTopOverflingDistance = mOverscrollHeaderView.getHeight();
		}
		if (mOverscrollFooterView != null) {
			mOverscrollFooterView.layout(
					mListPadding.left,
					mListPadding.top,
					mListPadding.left
							+ mOverscrollFooterView.getMeasuredWidth(),
					mOverscrollFooterView.getMeasuredHeight());
			mBottomOverflingDistance = mOverscrollFooterView.getHeight();
		}
	}
	...
	protected void layoutChildren() {
		...
		try {
			...
			default:
				...
				oldLast = getChildAt(childCount - 1);
			...
			if (mItemCount == 0) {
				...
				Class adapterClass = mAdapter.getClass();
				// 如果是HeaderViewAdpater则
				if (mAdapter instanceof HeaderViewListAdapter) {
					adapterClass = ((HeaderViewListAdapter) mAdapter)
							.getWrappedAdapter().getClass();
				}
			...
			if (dataChanged) {
				for (int i = 0; i < childCount; i++) {
					...
					// ViewDebug.RecyclerTraceType.MOVE_TO_SCRAP_HEAP, index,
					// i);
					// }
				}
			} else {
			...
			case LAYOUT_SYNC:
				if (!mStackFromBottom) {
					...
					sel = fillSpecificBottom(mSyncPosition, mLayoutHeight
							- mSpecificBottom);
				}
				break;
			case LAYOUT_FORCE_BOTTOM:
			...
			case LAYOUT_SPECIFIC:
				...
				break;
			case LAYOUT_SPECIFIC_BOTTOM:
				sel = fillSpecificBottom(reconcileSelectedPosition(),
						mLayoutHeight - mSpecificBottom);
				break;
			case LAYOUT_MOVE_SELECTION:
			...
			default:
				...
				// 一般大小没变, 只是触发了layout
					...
					if (mSelectedPosition >= 0
					...
					} else if ((mScrollY == 0 && !mStackFromBottom)
							|| mScrollY < 0) {
						...
						if (lastPosition < mItemCount) {
							sel = fillSpecificBottom(
									lastPosition,
									oldLast == null ? childrenBottom : oldLast
											.getBottom());
						} else {
							sel = fillSpecificBottom(mItemCount - 1,
									mLayoutHeight);
						}
					}
			...
			// 最后一个
			lastPosition = mFirstPosition + getChildCount() - 1;

			// 数据变了, 计算动画效果
			if (mInsertAnimation != null && mAddingRows != null && dataChanged
					&& mItemCount > 0 && lastPosition >= mItemCount - 1) {

				View lastChild = getChildAt(getChildCount() - 1);
				// 屏幕没充满
				if (lastChild.getBottom() < childrenBottom) {
					for (int row : mAddingRows) {
						if (row >= mFirstPosition && row <= lastPosition) {
							// 添加动画
							getChildAt(row - mFirstPosition).startAnimation(
									mInsertAnimation);
						}
					}
				}
	...
	public void setInsertAnimation(Animation mInsertAnimation) {
		this.mInsertAnimation = mInsertAnimation;
	}
	...
	private void setupChild(View child, int position, int y, boolean flowDown,
		...
		setupChild(child, position, y, flowDown, childrenLeft, selected,
				recycled, flowDown ? -1 : 0);
	}
	...
	@TargetApi(Build.VERSION_CODES.HONEYCOMB)
	private void setupChild(View child, int position, int y, boolean flowDown,
			int childrenLeft, boolean selected, boolean recycled, int index) {
		...
		if ((recycled && !p.forceAdd)
			...
			attachViewToParent(child, index, p);
		} else {
			p.forceAdd = false;
	 ...
	 */
	@TargetApi(Build.VERSION_CODES.HONEYCOMB)
	private int updateChild(View scrapView, int position, int y,
			boolean flowDown, int childrenLeft, int index) {
		// 旧的高度
		final int oldHeight = scrapView.getHeight();

		// 旧的type
		AbsListView.LayoutParams lp = (AbsListView.LayoutParams) scrapView
				.getLayoutParams();
		int oldViewType = lp.viewType;

		// 新的view
		View child = null;
		// type相同的话, 使用旧的view去获取
		...
		if (oldViewType == newViewType) {
			...
			child = mAdapter.getView(position,
					mRecycler.getScrapView(position), this);
		}
		...
		p.viewType = newViewType;
		// 如果和旧的不一样, 则移除旧的
		if (child != scrapView) {
			// 记录下旧的状态
			final boolean isSelected = scrapView.isSelected();
			final boolean isPressed = scrapView.isPressed();
			mRecycler.addScrapView(scrapView, position);
			if (mCacheColorHint != 0) {
				child.setDrawingCacheBackgroundColor(mCacheColorHint);
			}
			// 移除旧的..
			this.detachViewFromParent(index);
		...
		return child.getHeight() - oldHeight;
	}
	...
	public void setSelectionFromTop(int position, int y) {
		...
		if (position >= 0) {
			...
			// requestLayout会改变specificTop..所以要在之后设置...(貌似有问题?)
	 ...
	 *            从下网上算, 即如果在底部, 则为0
	 */
	public void setSelectionFromBottom(int position, int y) {
		if (mAdapter == null) {
			return;
		}
		...
		if (position >= 0) {
			requestLayout();
			mLayoutMode = LAYOUT_SPECIFIC_BOTTOM;
			...
			// requestLayout会改变specificTop..所以要在之后设置...(貌似有问题?)
			// mSpecificTop = mListPadding.top + y;
			mSpecificBottom = y + mListPadding.bottom;
		}
	}
	...
	private boolean commonKey(int keyCode, int count, KeyEvent event) {
		...
		if (action != KeyEvent.ACTION_UP) {
			...
			case KeyEvent.KEYCODE_DPAD_UP:
				if (SDKVersionUtilsKt.isHoneycomb() && event.hasNoModifiers()) {
	...
	public boolean isOpaque() {
		...
		// mDividerIsOpaque) || super.isOpaque();
		// if (retValue)
		// {
		...
		// return false;
		// }
		// final int listBottom = getHeight() - (mListPadding != null ?
	...
	void drawOverscrollHeader(Canvas canvas, View view, Drawable shadow,
			Drawable drawable, Rect bounds) {
		int count = canvas.save();
		// 先画阴影
		if (shadow != null) {
			final int height = shadow.getIntrinsicHeight();
			if (height < bounds.height()) {
				final Rect newBounds = new Rect(bounds);
				// 靠底部对齐
				newBounds.top = newBounds.top + bounds.height() - height;
		...
		if (view != null) {
			...
			view.offsetTopAndBottom(bounds.bottom - view.getBottom());
			drawChild(canvas, view, getDrawingTime());
		}
		canvas.restoreToCount(count);
	}

	void drawOverscrollFooter(Canvas canvas, View view, Drawable drawable,
			Rect bounds) {
		int count = canvas.save();
		// 再画图
		if (drawable != null) {
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		final View overScrollHeaderView = mOverscrollHeaderView;
		final View overScrollFooterView = mOverscrollFooterView;
		final Drawable overscrollFooter = mOverScrollFooter;
		final Drawable overScrollHeaderShadow = mOverScrollHeaderShadow;
		final boolean drawOverscrollHeader = (overscrollHeader != null
				|| overScrollHeaderView != null || overScrollHeaderShadow != null);
		final boolean drawOverscrollFooter = (overscrollFooter != null || overScrollFooterView != null);
		final boolean drawDividers = dividerHeight > 0 && mDivider != null;

		// 画contentBackground
		if (mContentBackgroundDrawable != null) {
			// 上拉刷新的话. 下面也要填充
			if (mScrollY > 0) {
				mContentBackgroundDrawable.setBounds(0, mScrollY, getWidth(),
						getHeight() + mScrollY);
			} else {
				mContentBackgroundDrawable.setBounds(0, 0, getWidth(),
						getHeight());
			}
			mContentBackgroundDrawable.draw(canvas);
		}
		...
		if (drawDividers || drawOverscrollHeader || drawOverscrollFooter) {
			...
			final boolean fillForMissingDividers = SDKVersionUtilsKt.isECLAIR_MR1() ? (isOpaque() && !super
					.isOpaque()) : isOpaque();
			...
			if (count > 0) {
				...
				if (scrollY < 0) {
					if (drawOverscrollHeader) {
						bounds.bottom = 0 - mDividerHeight;
						bounds.top = scrollY;
						drawOverscrollHeader(canvas, overScrollHeaderView,
								overScrollHeaderShadow, overscrollHeader,
								bounds);
					}
				} else if (scrollY > 0) {
					if (drawOverscrollFooter) {
						int overFooterBottom = mBottom + scrollY
								+ mDividerHeight;
						bounds.top = mBottom + mDividerHeight;
						bounds.bottom = overFooterBottom;
						drawOverscrollFooter(canvas, overScrollFooterView,
								overscrollFooter, bounds);
					}
				}
				if (drawDividers) {
					bounds.bottom = 0;
			...
			// 画OverScroller Footer
			// final int overFooterBottom = mBottom + scrollY + mDividerHeight;
			// if (drawOverscrollFooter && first + count == itemCount
			// && overFooterBottom > mBottom) {
			// bounds.top = mBottom + mDividerHeight;
			// bounds.bottom = overFooterBottom;
	 ...
	 * {@link #setDividerHeight(int)} 不建议使用ColorDrawable, 在2,2之前的版本会有bug
	 * 
	 * @param divider
	 *            The drawable to use.
	 */
	public void setDivider(Drawable divider) {
		...
		mDividerHeight = mDividerHeight < 0 ? 0 : mDividerHeight;
		mDivider = divider;
	...
	// /* (non-Javadoc)
	// *
	// * Look in the passed in list of headers or footers for the first view
	// that matches
	// * the predicate.
	// */
	 ...
	 * 
	 * @param view
	 *            下拉刷新的Header, 如果要清除Header, 传null
	 */
	public void setOverScrollHeader(View view) {
		if (view == null) {
			if (mOverscrollHeaderView != null) {
				mOverscrollHeaderView.removeAllViewsInLayout();
				mOverscrollHeaderView.setParent(null);
				mOverscrollHeaderView = null;
			}
		} else {
			if (mOverscrollHeaderView == null) {
				mOverscrollHeaderView = new OverscrollViewContainer(
						this.getContext());
				mOverscrollHeaderView.setParent(this);
			}
			mOverscrollHeaderView.removeAllViewsInLayout();
			mOverscrollHeaderView.addView(view);
		}
		// attachWindow(mOverscrollHeaderView);

		if (DEBUG) {
			// Log.d(TAG, "OVERSCROLL_STATUS_IDLE");
		}
		mOverscrollHeadState = OVERSCROLL_STATUS_IDLE;
		mScrollY = 0;
	}
	...
	public void setOverScrollFooter(View view) {
		if (view == null) {
			if (mOverscrollFooterView != null) {
				mOverscrollFooterView.removeAllViewsInLayout();
				mOverscrollFooterView.setParent(null);
				mOverscrollFooterView = null;
			}
		} else {
			if (mOverscrollFooterView == null) {
				mOverscrollFooterView = new OverscrollViewContainer(
						this.getContext());
				mOverscrollFooterView.setParent(this);
			}
			mOverscrollFooterView.removeAllViewsInLayout();
			mOverscrollFooterView.addView(view);
		}
		// attachWindow(mOverscrollHeaderView);
	...
	public View getOverScrollHeaderView() {
		return mOverscrollHeaderView;
	}

	public View getOverScrollFooterView() {
		return mOverscrollFooterView;
	}
	 ...
	 * @param listener
	 */
	public void setOverScrollListener(OverScrollViewListener listener) {
		this.mOverScrollViewListener = listener;
	}

	protected int getSpringbackOffset() {
		final int scorllY = getScrollY();
		// head显示
		if (mOverscrollHeaderView != null && scorllY < 0) {
			// 完全显示下释放
			if (scorllY <= -getOverScrollHeight()) {
				// 通知
				boolean hold = false;
				if (mOverscrollHeadState == OVERSCROLL_STATUS_COMPLETE_VISABLE) {
					if (mOverScrollViewListener != null) {
						hold = mOverScrollViewListener
								.onViewCompleteVisableAndReleased(
										OverScrollViewListener.OVERSCROLL_POSITION_TOP,
										mOverscrollHeaderView.getChildAt(0),
										this);
					}
					mOverscrollHeadState = OVERSCROLL_STATUS_COMPLETE_RELEASE;
					if (DEBUG) {
						// Log.d(TAG, "OVERSCROLL_STATUS_COMPLETE_RELEASE");
					}
				}
				if (hold) {
					return mOverscrollHeaderView != null ? -getOverScrollHeight()
							: 0;
				} else {
					if (DEBUG) {
						// Log.d(TAG, "OVERSCROLL_STATUS_IDLE");
			...
			// 非完全显示下释放
				...
				// 通知
				if (mOverscrollHeadState < OVERSCROLL_STATUS_COMPLETE_VISABLE
						|| mEnsureOverScrollStatusToIdleWhenRelease) {
					if (mOverScrollViewListener != null) {
						mOverScrollViewListener
								.onViewNotCompleteVisableAndReleased(
										OverScrollViewListener.OVERSCROLL_POSITION_TOP,
										mOverscrollHeaderView.getChildAt(0),
										this);
					}
		...
		// footer显示
		if (mOverscrollFooterView != null && scorllY > 0) {
			// 完全显示下释放
			if (scorllY > getOverScrollFooterHeight()) {
				// 通知
				boolean hold = false;
				if (mOverscrollHeadState == OVERSCROLL_STATUS_COMPLETE_VISABLE) {
					if (mOverScrollViewListener != null) {
						hold = mOverScrollViewListener
								.onViewCompleteVisableAndReleased(
										OverScrollViewListener.OVERSCROLL_POSITION_BOTTOM,
										mOverscrollFooterView.getChildAt(0),
										this);
					}
					mOverscrollHeadState = OVERSCROLL_STATUS_COMPLETE_RELEASE;
					if (DEBUG) {
				...
				if (hold) {
					return mOverscrollFooterView != null ? getOverScrollFooterHeight()
							: 0;
				} else {
					if (DEBUG) {
						// Log.d(TAG, "OVERSCROLL_STATUS_IDLE");
	...
	/**
	 * 检测OverScroller Header是否完全显示
	 */
	private void checkOverScrollHeaderIsVisable() {
		// 更新overScroll的状态
		if (mTouchMode == TOUCH_MODE_OVERSCROLL && getScrollY() != 0) {
			final int scrollY = getScrollY();
			if (scrollY < 0
					&& mOverscrollHeaderView != null
					&& scrollY > -getOverScrollHeight()
					&& (mOverscrollHeadState == OVERSCROLL_STATUS_IDLE || mOverscrollHeadState == OVERSCROLL_STATUS_COMPLETE_VISABLE)) {

				if (mOverScrollViewListener != null) {
					mOverScrollViewListener.onNotCompleteVisable(
							OverScrollViewListener.OVERSCROLL_POSITION_TOP,
							mOverscrollHeaderView.getChildAt(0), this);
				}
				if (DEBUG) {
					// Log.d(TAG, "OVERSCROLL_STATUS_VISABLE");
				}
				mOverscrollHeadState = OVERSCROLL_STATUS_VISABLE;
			}
		}
	}
	...
	private void checkOverscrollViewIsCompleteVisable(View view) {
		...
		if (view == mOverscrollHeaderView) {
			// 完全显示
			if ((mOverscrollHeadState == OVERSCROLL_STATUS_VISABLE)
					&& scorllY <= -getOverScrollHeight()) {
				mOverscrollHeadState = OVERSCROLL_STATUS_COMPLETE_VISABLE;
				if (DEBUG) {
					// Log.d(TAG, "OVERSCROLL_STATUS_COMPLETE_VISABLE");
				}
				if (mOverScrollViewListener != null) {
					mOverScrollViewListener.onViewCompleteVisable(
							OverScrollViewListener.OVERSCROLL_POSITION_TOP,
							mOverscrollHeaderView.getChildAt(0), this);
				}
			}
	...
	private void checkOverScrollFooterIsVisable() {
		...
		if (mTouchMode == TOUCH_MODE_OVERSCROLL && getScrollY() != 0) {
			...
			if (scrollY > 0
					&& mOverscrollFooterView != null
					&& scrollY < getOverScrollFooterHeight()
					&& (mOverscrollHeadState == OVERSCROLL_STATUS_IDLE || mOverscrollHeadState == OVERSCROLL_STATUS_COMPLETE_VISABLE)) {

				if (mOverScrollViewListener != null) {
					mOverScrollViewListener.onNotCompleteVisable(
							OverScrollViewListener.OVERSCROLL_POSITION_BOTTOM,
							mOverscrollFooterView.getChildAt(0), this);
				}
				if (DEBUG) {
					// Log.d(TAG, "OVERSCROLL_STATUS_VISABLE");
				}
	...
	private void checkOverscrollFooterViewIsCompleteVisable(View view) {
		final int scorllY = getScrollY();
		final int viewHeight = view.getHeight();
		if (view == mOverscrollFooterView) {
			// 完全显示
			if ((mOverscrollHeadState == OVERSCROLL_STATUS_VISABLE)
					&& scorllY > getOverScrollFooterHeight()) {
				mOverscrollHeadState = OVERSCROLL_STATUS_COMPLETE_VISABLE;
				if (DEBUG) {
					// Log.d(TAG, "OVERSCROLL_STATUS_COMPLETE_VISABLE");
				}
	...
	private boolean isTouchHolding = false;

	/**
	 * 回弹并隐藏header
	 */
	public void springBackOverScrollHeaderView() {
		if (getScrollY() < 0 && !isTouchHolding) {
			doSpringBack();
		}
		if (DEBUG) {
			// Log.d(TAG, "OVERSCROLL_STATUS_IDLE");
	...
	public void hideOverScrollHeaderView() {
		if (mScrollY < 0) {
			...
			// 先请求更新一下...
			if (!mStackFromBottom) {
				setSelectionFromTop(mFirstPosition, mListPadding.top - mScrollY);
			} else {
				// 从第一行的下面开始布局
				mLayoutMode = LAYOUT_SPECIFIC_BOTTOM;
				View view = getChildAt(0);
				int bottom = view == null ? 0 : (mLayoutHeight
						- view.getBottom() - mListPadding.bottom);
				setSelectionFromBottom(mFirstPosition, bottom + mScrollY);
			}
	...
	public ViewParent invalidateChildInParent(int[] location, Rect dirty) {
		...
		if (!(dirty.bottom <= 0 || dirty.top >= getHeight())) {
			final int scrollY = getScrollY();
			// 在有硬件加速(3.0以后), 由于displaylist会缓存住绘图缓冲
			// 而header并没有实际添加进ListView中. 因此当header有动画时候, 实际上不会进行重绘, 而是将缓冲贴上去,
			// 造成动画掉帧的感觉
			// header被重绘, 需要把自身添加invalidate标记.
			// 否则无法触发dispatchDraw
			if (scrollY < 0 && dirty.top + scrollY < 0) {
				invalidateWithoutNotiyParent(dirty);
			} else if (scrollY > 0 && dirty.bottom > getHeight() - scrollY) {
				invalidateWithoutNotiyParent(dirty);
			}
		}
		return ret;
	...
	private void invalidateWithoutNotiyParent(Rect dirty) {
		try {
			Field f = View.class.getDeclaredField("mPrivateFlags");
			f.setAccessible(true);
			f.set(this, f.getInt(this) | 0x80000000 /* INVALIDATED */);
		} catch (Exception e) {
			Log.printErrStackTrace("ListView", e, null, null);
			// if (QLog.isColorLevel()) {
			// QLog.e(TAG, QLog.CLR, e.getMessage(), e);
			// }
	...
	// hack.
	// 调用dispatchAttachedToWindow方法. 否则会收不到drawable回调
	private void attachWindow(View view) {
		if (DEBUG) {
			// Log.d(TAG, "Overscroll view was attach to Window");
		}
		try {
			Field f = View.class.getDeclaredField("mAttachInfo");
			f.setAccessible(true);
			Object attachInfo = f.get(this);
			if (attachInfo != null) {
				Method m = View.class.getDeclaredMethod(
						"dispatchAttachedToWindow", attachInfo.getClass(),
						Integer.TYPE);
				m.setAccessible(true);
				m.invoke(view, attachInfo, getVisibility());
			}
		} catch (Exception e) {
			Log.printErrStackTrace("ListView", e, null, null);
			// if (QLog.isColorLevel()) {
	...
	private void detachedWindow(View view) {
		if (DEBUG) {
			// Log.d(TAG, "Overscroll view was detached from window");
		}
		try {
			Field f = View.class.getDeclaredField("mAttachInfo");
			f.setAccessible(true);
			Object attachInfo = f.get(view);
			if (attachInfo != null) {
				Method m = View.class
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		// 这里有hack, 因为Header并不是在ListView的Child里
		// 这里需要把Header attachToWindow, 否则, Header中动画的回调将不起作用
		// 2.3以下有bug
		if (t < 0 && oldt == 0 && mOverscrollHeaderView != null) {
			attachWindow(mOverscrollHeaderView);
		} else if (t == 0 && oldt < 0 && mOverscrollHeaderView != null) {
			detachedWindow(mOverscrollHeaderView);
		}
	}

	private static class OverscrollViewContainer extends FrameLayout {

		public OverscrollViewContainer(Context context) {
			super(context);
		}

		private void setParent(ViewParent parent) {
			mParent = parent;
		}
	}
	...
	private static final int ANIMATION_TAG = 0x7f08006d;
	// private static final int DEL_ANIM_TAG = 0x7f08006e;
	private static final int ANIMATION_DURATION = 350;

	private long delAnimDuration;

	public void setDelAnimationDuration(long d) {
		if (d > 0) {
			delAnimDuration = d;
		}
	}
	...
	public long getDelAnimationDuration() {
		if (delAnimDuration > 0) {
			return delAnimDuration;
		} else {
	...
	public void clearDelAnim() {
		int lvChildCount = getChildCount();
		for (int i = 0; i < lvChildCount; i++) {
			View child = getChildAt(i);
	...
	class ListDataSetObserver extends AdapterDataSetObserver implements
			ListDataSetListener {

		@Override
		public void onRowInserted(int first, int last) {
			// 插到最前面, 用原来的实现
			// 如果为0的话, 需要加动画, 因此还是走notifyDataSetChanged
			if (first == 0 && mItemCount > 0) {
				long current = System.nanoTime();
				if (last < first) {
					throw new IllegalArgumentException(
							"lastRow must more than firstRow!");
				}

				if (first < 0
						|| first > mItemCount - getHeaderViewsCount()
								- getFooterViewsCount()) {
					throw new IllegalArgumentException("row index out of bound");
				}

				// 如果mDataChanged还没改变,则说明即将进行下一次layout
				// 所以只需要调用onChange通知即可
				if (mNeedSync || mDataChanged) {
					this.onChanged();
					return;
				}
				...
				// 如果有设置foot或者header, 加上headerViewCount
				first += getHeaderViewsCount();
				last += getHeaderViewsCount();

				// 要添加的行数
				final int insertCount = last - first + 1;
				int childCount = getChildCount();
				final int firstPosition = mFirstPosition;
				final int lastPosition = mFirstPosition + childCount - 1;

				final boolean insertAtLast = (first == mItemCount);

				// 通知fastScroller, itemCount改变
				if (mFastScroller != null) {
					mFastScroller.onItemCountChanged(mItemCount, mItemCount
							- insertCount);
				}
				// 更新itemCount
				mItemCount = mItemCount + insertCount;

				// 更新selected_position
				if (mSelectedPosition >= 0) {
					int newPosition = calcNewPositionAdd(mSelectedPosition,
							first, last, firstPosition, lastPosition);
					setSelectedPositionInt(newPosition);
					setNextSelectedPositionInt(newPosition);
				} else if (mSelectorPosition >= 0) {
					mSelectedPosition = calcNewPositionAdd(mSelectorPosition,
							first, last, firstPosition, lastPosition);
				}
				if (mMotionPosition >= 0) {
					mMotionPosition = calcNewPositionAdd(mMotionPosition,
							first, last, firstPosition, lastPosition);
				}
				int checkCount = mCheckStates == null ? 0 : mCheckStates.size();
				// 更新checked
				if (checkCount > 0) {
				...
				// 更新insertRow
				if (mAddingRows != null) {
					for (int i = 0; i < mAddingRows.length; i++) {
						int position = mAddingRows[i];
						// 需要修改
						if (position >= first) {
							position += (last - first + 1);
							mAddingRows[i] = position;
						}
					}
				}
				...
				// 插到最后
				final boolean showLast = (getTranscriptMode() == TRANSCRIPT_MODE_NORMAL
						&& insertAtLast && lastBottom <= childrenBottom);

				// 插入位置在第一个显示之前.
				if (first <= firstPosition && !showLast
						&& lastBottom >= childrenBottom) {
					mFirstPosition += insertCount;
					return;
				}
				// 插入位置在最后一个之后
				else if (first > lastPosition && !showLast
						&& lastBottom >= childrenBottom) {
					return;
				}
				// 在屏幕内, 开始移动
					...
					// 当前选中的位置
					final int selection = mSelectedPosition >= 0 ? mSelectedPosition
							: (shouldShowSelector() ? mSelectorPosition
									: INVALID_POSITION);
					// 插入的位置
					final int insertIndex = first - firstPosition;
					// 修改的索引
					// final int insertPosition = (first < firstPosition ) ? 0 :
					// first - firstPosition;
					// final int updateLastIndex = (last > lastPosition ) ?
					// childCount - 1 : last - firstPosition;
					// 插入到最后
					if (showLast) {
						int bottom = childrenBottom;
						int totalOffset = 0;
						int tmp = -1;

						// 执行插入, 并计算插入的高度
						for (int i = last; i >= first; i--) {
							if (bottom < childrenTop) {
								tmp = i + 1;
								break;
							}
							// 创建一个view
							// View view = makeAndAddView(i, bottom, false,
							...
							// This needs to be positioned and measured
							setupChild(view, i, bottom, false,
									mListPadding.left, false, mIsScrap[0],
									insertIndex);

							int height = view.getHeight() + mDividerHeight;
							bottom -= height;
							totalOffset += height;
						}
						// 修改的部分已经塞满了整个屏幕, 把前面全删了
						if (tmp >= 0) {
							for (int i = 0; i <= insertIndex - 1; i++) {
								mRecycler.addScrapView(getChildAt(i), i
										+ mFirstPosition);
							}
							detachViewsFromParent(0, insertIndex);
							mFirstPosition = tmp;
						} else {
							offsetTop(childrenTop, insertIndex - 1, totalOffset);
						}
					}
					// 插入点上半部分固定
					else if ((selection >= 0 && selection < first)
							|| (selection < 0 && !mStackFromBottom)) {
						int top = childrenTop;
						int totalOffset = 0;
						int tmp = -1;
						// 插入的坐标
						if (insertIndex > 0) {
							// 理论上不可能出现...
							top = getChildAt(insertIndex - 1).getBottom()
									+ mDividerHeight;
						}
						// 插在第一个的上面. 可以不用显示了...
						else {
						...
						for (int i = first; i <= last; i++) {
							if (top > childrenBottom) {
								tmp = i - mFirstPosition;
								break;
							}
							// 创建一个view
							// View view = makeAndAddView(i, top, true,
							// mListPadding.left, false);
							...
							// This needs to be positioned and measured
							setupChild(view, i, top, true, mListPadding.left,
									false, mIsScrap[0], i - mFirstPosition);
						...
						// 修改的部分已经塞满了整个屏幕, 把后面全删了
						if (tmp >= 0) {
							for (int i = tmp; i < childCount; i++) {
								mRecycler.addScrapView(getChildAt(i), i
										+ mFirstPosition);
							}
							detachViewsFromParent(tmp, childCount - tmp);
						} else {
							offsetBottom(getChildCount(), childrenBottom, last
									- mFirstPosition + 1, totalOffset);
						}
					}
					// 插入点下半部分固定
					else {
						// 如果没满, 先移下来
						if (childrenBottom > lastBottom) {
							offsetChildrenTopAndBottom(childrenBottom
									- lastBottom);
						}
						int bottom = getChildAt(insertIndex).getTop()
								- mDividerHeight;
						int totalOffset = 0;
						int tmp = -1;
				...
				if (DEBUG) {
					// Log.d(TAG, "ListView onRowDelete cost: " +
					// (((float)(System.nanoTime() - current))/1000000) +
					// " ms");
				}
			...
			// 多次通知
				...
				// 旧的也add进去
				int[] rows = mAddingRows;
				for (int i : rows) {
					set.add(i);
				}
				// 转成数组
				rows = new int[set.size()];
				int tmp = 0;
				// 转成int[]
				for (int i : set) {
					rows[tmp++] = i;
		...
		@Override
		public void onRowDeleted(int... delted) {
			// 先挖个坑
			this.onChanged();
		}

		@Override
		public void onRowDeleted(int first, int last) {
			long current = System.nanoTime();
			if (last < first) {
				throw new IllegalArgumentException(
						"lastRow must more than firstRow!");
			...
			if (first < 0
					|| last >= mItemCount - getHeaderViewsCount()
							- getFooterViewsCount()) {
				throw new IllegalArgumentException("row index out of bound");
			}
			...
			if (mFastScroller != null) {
				mFastScroller
						.onItemCountChanged(mItemCount, mItemCount - count);
			}
			// 更新itemCount
			mItemCount = mItemCount - count;

			// 更新selected_position
			if (mSelectedPosition >= 0) {
				int newPosition = calcNewPosition(mSelectedPosition, first,
						last, firstPosition, lastPosition);
				setSelectedPositionInt(newPosition);
				setNextSelectedPositionInt(newPosition);
			} else if (mSelectorPosition >= 0) {
				mSelectedPosition = calcNewPosition(mSelectorPosition, first,
						last, firstPosition, lastPosition);
			}
			if (mMotionPosition >= 0) {
				mMotionPosition = calcNewPosition(mMotionPosition, first, last,
			...
			if (checkCount > 0) {
				for (int i = 0; i < checkCount; i++) {
					...
					if (position >= first) {
						...
						// 在删除的后面
						if (position > last) {
							position = position - (last - first + 1);
							// 把新值添加到后面
							mCheckStates.put(position, value);
						}
					}
			...
			// 删除的部分在屏幕外, 不做任何改变
			if (last < firstPosition || first > lastPosition) {
				return;
			}
			// 在屏幕内, 开始移动
			else {
				...
				final int removeFirstIndex = (first < firstPosition) ? 0
						: first - firstPosition;
				final int removeLastIndex = (last > lastPosition) ? childCount - 1
						: last - firstPosition;

				// 移掉所有删掉的view
				// for (int i = removeFirstIndex; i <= removeLastIndex; i++)
				for (int i = 0; i < childCount; i++) {
					View child = getChildAt(i);
					if (i >= removeFirstIndex && i <= removeLastIndex) {
						mRecycler.addScrapView(child, i);
						child.setTag(ANIMATION_TAG, null);
					} else if (mAnimation) {
						child.setTag(ANIMATION_TAG, new Point(child.getTop(),
								child.getBottom()));
					}
				}
				detachViewsFromParent(removeFirstIndex, removeLastIndex
						- removeFirstIndex + 1);
				...
				// 如果有选中, 则以选中的View为基准
				// boolean hasSelection = (mSelectedPosition >= 0 ||
				// mSelectorPosition >= 0);
				boolean shouldShowSelector = shouldShowSelector();
				final int newSelectionPosition = mSelectedPosition >= 0 ? mSelectedPosition
						: (shouldShowSelector ? mSelectorPosition
								: INVALID_POSITION);

				boolean fillDown = true;
				boolean deleteAllVisable = false;

				// if (QLog.isColorLevel()) {
				// QLog.d(TAG, QLog.CLR, "first is:" + first + ",last is:"
				// + last + ",firstPosition is:" + firstPosition
				// + ",lastPosition is:" + lastPosition
				// + ",newSelectionPosition is:"
				// + newSelectionPosition + ",mStackFromBottom is:"
				// + mStackFromBottom);
				// }
				int upItemsMoveDistance = 0;
				int downItemsMoveDistance = 0;
				int upItemsMaxIndex = -1;
				// 当前能显示全删了
				if (first <= firstPosition && last >= lastPosition) {
					// 把下面的往上拉
					if (!mStackFromBottom) {
						// 新的firstPosition的值.
						final int syncPosition = first > mItemCount - 1 ? mItemCount - 1
								: first;
						fillSpecific(syncPosition, childrenTop);
					} else {
						fillSpecificBottom(first - 1 < 0 ? 0 : first - 1,
								childrenBottom);
					}
					deleteAllVisable = true;
				}
				// 删除某一行之上所有的
				else if (first <= firstPosition) {
					if (mStackFromBottom || newSelectionPosition >= 0) {
						mFirstPosition = first;
						View baselineView = getChildAt(removeFirstIndex);
						int baselineViewOldTop = baselineView.getTop();
						// 填补上面的空缺
						fillDown = false;
						fillGap(fillDown);
						stayOnTheTop();
						int baselineViewNewTop = baselineView.getTop();
						downItemsMoveDistance = baselineViewOldTop
								- baselineViewNewTop;
						upItemsMoveDistance = baselineViewNewTop
								- mDividerHeight > 0 ? -baselineViewNewTop
								+ mDividerHeight : 0;
						upItemsMaxIndex = indexOfChild(baselineView) - 1;
					} else {
						final View firstChild = getChildAt(0);
						int firstChildOldTop = firstChild.getTop();
						offsetChildrenTopAndBottom(childrenTop
								- firstChild.getTop());
						mFirstPosition = first;
						fillDown = true;
						fillGap(fillDown);
						stayOnTheTop();
						upItemsMoveDistance = firstChild.getTop()
								- mDividerHeight > 0 ? -firstChild.getTop()
								+ mDividerHeight : 0;
						downItemsMoveDistance = firstChildOldTop
								- firstChild.getTop();
						upItemsMaxIndex = indexOfChild(firstChild);

					}
				}
				// 删除某一行之下所有的
				else if (last >= lastPosition) {
					if (!mStackFromBottom || newSelectionPosition >= 0) {
						View lastChild = getChildAt(childCount - 1);
						int lastChildOldBottom = lastChild.getBottom();
						fillDown = true;
						fillGap(fillDown);
						stayOnTheTop();
						int lastChildNewBottom = lastChild.getBottom();
						upItemsMoveDistance = lastChildOldBottom
								- lastChildNewBottom;
						downItemsMoveDistance = childrenBottom
								- lastChildNewBottom - mDividerHeight > 0 ? childrenBottom
								- lastChildNewBottom - mDividerHeight
								: 0;
						upItemsMaxIndex = indexOfChild(lastChild);
					} else {
						View lastChild = getChildAt(childCount - 1);
						int lastChildOldTop = lastChild.getTop();
						// 剩下的移到最下面去
						offsetChildrenTopAndBottom(childrenBottom
								- lastChild.getBottom());
						fillDown = false;
						...
						stayOnTheTop();
						downItemsMoveDistance = childrenBottom
								- lastChild.getBottom() - mDividerHeight > 0 ? childrenBottom
								- lastChild.getBottom() - mDividerHeight
								: 0;
						upItemsMoveDistance = lastChildOldTop
								- lastChild.getTop();
						upItemsMaxIndex = indexOfChild(lastChild);
					}
				}
				// 删除中间的
					...
					if ((newSelectionPosition >= 0 && newSelectionPosition < first)
							|| (newSelectionPosition < 0 && !mStackFromBottom)) {
						View removeUpView = getChildAt(removeFirstIndex - 1);
						int removeUpViewOldTop = removeUpView.getTop();
						View baselineView = getChildAt(removeFirstIndex);
						int baselineViewOldTop = baselineView.getTop();
						// 下面的上移
						int gapHeight = baselineView.getTop()
								- removeUpView.getBottom();
						for (int i = first - firstPosition; i < childCount; i++) {
							getChildAt(i).offsetTopAndBottom(
									-gapHeight + mDividerHeight);
						}
						// 下面的填充
						fillDown = true;
						fillGap(fillDown);
						...
						downItemsMoveDistance = baselineViewOldTop
								- baselineView.getTop();
						upItemsMoveDistance = removeUpViewOldTop
								- removeUpView.getTop();
						upItemsMaxIndex = indexOfChild(removeUpView);
					} else if (newSelectionPosition >= first
							|| (newSelectionPosition < 0 && mStackFromBottom)) {
						View removeUpView = getChildAt(removeFirstIndex - 1);
						int removeUpViewOldTop = removeUpView.getTop();
						View baselineView = getChildAt(removeFirstIndex);
						...
						// 上面的下移
						int gapHeight = baselineView.getTop()
								- getChildAt(removeFirstIndex - 1).getBottom();
						for (int i = 0; i < first - firstPosition; i++) {
							getChildAt(i).offsetTopAndBottom(
									gapHeight - mDividerHeight);
						}
						fillDown = false;
				...
				// 不在touchmode下, 更新焦点的位置
				if (newSelectionPosition >= 0
						&& newSelectionPosition < childCount
						&& shouldShowSelector) {
					positionSelector(newSelectionPosition,
							getChildAt(newSelectionPosition - mFirstPosition));
				}

				// if (QLog.isColorLevel()) {
				// QLog.d(TAG, QLog.CLR, "upItemsMaxIndex is:"
				// + upItemsMaxIndex + ",upItemsMoveDistance is:"
				// + upItemsMoveDistance
				// + ",downItemsMoveDistance is:"
				// + downItemsMoveDistance);
				// }
				// 计算动画
				if (mAnimation) {
					calcAnimation(childCount, upItemsMaxIndex,
							upItemsMoveDistance, downItemsMoveDistance,
							deleteAllVisable);
				}
			}
			invalidate();
		...
		@Override
		public void onRowUpdated(int first, int last) {
			if (last < first) {
				throw new IllegalArgumentException(
						"lastRow must more than firstRow!");
			}

			if (first < getHeaderViewsCount()
					|| last >= mItemCount - getHeaderViewsCount()
							- getFooterViewsCount()) {
				throw new IllegalArgumentException(
						"row index out of bound."
								...
								+ getHeaderViewsCount()
								+ "~"
								+ (mItemCount - getHeaderViewsCount() - getFooterViewsCount()));
			}
			...
			// 在屏幕内, 开始移动
				...
				// 修改的索引
				final int updateFirstIndex = (first < firstPosition) ? 0
						: first - firstPosition;
				final int updateLastIndex = (last > lastPosition) ? childCount - 1
						: last - firstPosition;
				...
				// 选中的位置在修改之前, 或者上往下布局
				if ((selection >= 0 && selection <= first)
						|| (selection < 0 && !mStackFromBottom)) {
					// 更新
					int offset = updateAfter(childCount, childrenBottom,
							updateFirstIndex, updateLastIndex);
					correctTooLow(getChildCount());
					// 剩下的移动
					offsetBottom(childCount, childrenBottom,
							updateLastIndex + 1, offset);
				}
				// 选中的位置在修改之后, 或者是从下网上布局
				else if (selection >= last
						|| (selection < 0 && mStackFromBottom)) {
					int totalOffset = updateBefore(childrenTop,
							updateFirstIndex, updateLastIndex);
					correctTooHigh(getChildCount());
					offsetTop(childrenTop, updateFirstIndex - 1, totalOffset);
				}
				// 选中的位置在修改的之间
				else {
					int selectionIndex = selection - mFirstPosition;
					int offset = updateAfter(childCount, childrenBottom,
							selectionIndex, updateLastIndex);
					offsetBottom(childCount, childrenBottom,
							updateLastIndex + 1, offset);

					offset = updateBefore(childrenTop, updateFirstIndex,
							selectionIndex - 1);
					offsetTop(childrenTop, updateFirstIndex - 1, offset);
				}
				stayOnTheTop();
			}
			if (DEBUG) {
				// Log.d(TAG, "ListView onRowUpdate cost: " +
				// (((float)(System.nanoTime() - current))/1000000) + " ms");
			}
		}

		private int updateBefore(int childrenTop, final int updateFirstIndex,
				final int updateLastIndex) {
			// update的View一共修改改变了多少height
			int totalOffset = 0;
			int bottom = Integer.MAX_VALUE;
			int tmp = -1;
			for (int i = updateLastIndex; i >= updateFirstIndex; i--) {
				...
				int oldHeight = child.getHeight();
				int offset = updateChild(child, i + mFirstPosition, bottom,
						false, mListPadding.left, i);
				totalOffset += offset;
				bottom -= oldHeight + offset + mDividerHeight;
			}

			if (tmp >= 0) {
				for (int i = 0; i <= tmp; i++) {
					mRecycler.addScrapView(getChildAt(i), i + mFirstPosition);
				}
				detachViewsFromParent(0, tmp + 1);
				mFirstPosition += tmp + 1;
				return 0;
			}
			return totalOffset;
		}

		private void offsetTop(int childrenTop, final int last, int totalOffset) {
			if (last < 0) {
				return;
			}
			// size变大了, 上面的view往上移动
			if (totalOffset > 0) {
				...
				for (int i = last; i >= 0; i--) {
					View child = getChildAt(i);
					// 出轨了
					if (child.getBottom() - totalOffset < childrenTop) {
						// 添加进scrap
						mRecycler.addScrapView(child, i + mFirstPosition);
						// 标记被移除的起始位置
						if (removed < 0) {
							removed = i;
						}
					...
					child.offsetTopAndBottom(-totalOffset);
				}
				// 多余的移除掉
				if (removed >= 0) {
					detachViewsFromParent(0, removed + 1);
					mFirstPosition += removed + 1;
				}
		...
		private int updateAfter(int childCount, int childrenBottom,
				final int updateFirstIndex, final int updateLastIndex) {
			// update的View一共修改改变了多少height
			int totalOffset = 0;
			...
			for (int i = updateFirstIndex; i <= updateLastIndex; i++) {
				...
				int oldHeight = child.getHeight();
				int offset = updateChild(child, i + mFirstPosition, top, true,
						mListPadding.left, i);
				;
				totalOffset += offset;
				top += oldHeight + offset + mDividerHeight;
			}
			// 修改的部分已经塞满了整个屏幕, 把后面全删了
			if (tmp >= 0) {
				for (int i = tmp; i < childCount; i++) {
		...
		// 下半部分进行位移. 并填充空白区域
		private void offsetBottom(int childCount, int childrenBottom,
				final int first, int totalOffset) {
			// size变大了, 后面的往下移
			if (totalOffset > 0) {
				int removed = -1;
				for (int i = first; i < childCount; i++) {
					View child = getChildAt(i);
					// 出轨了
					if (child.getTop() + totalOffset > childrenBottom) {
						// 添加进scrap
						mRecycler.addScrapView(child, i + mFirstPosition);
						// 标记被移除的起始位置
						if (removed < 0) {
				...
				// 多余的移除掉
				if (removed >= 0) {
					detachViewsFromParent(removed, childCount - removed);
				}
			}
			// size 变小了, 后面的往上移动
			else if (totalOffset < 0) {
		...
		// 计算动画
		private void calcAnimation(int childCount, int upItemsMaxIndex,
				int upItemsMoveDistance, int downItemsMoveDistance,
				boolean deleteAllVisable) {
			if (deleteAllVisable) {
				for (int i = 0; i < childCount; i++) {
					View child = getChildAt(i);
					AlphaAnimation alpha = new AlphaAnimation(0, 1);
					alpha.setDuration(getDelAnimationDuration());
					child.setAnimation(alpha);
				}
			} else {
				for (int i = 0; i <= upItemsMaxIndex; i++) {
					View child = getChildAt(i);
					TranslateAnimation a = new TranslateAnimation(0, 0,
							upItemsMoveDistance, 0);
					a.setDuration(getDelAnimationDuration());
					child.setAnimation(a);
				}
				for (int i = upItemsMaxIndex + 1; i < childCount; i++) {
					View child = getChildAt(i);
					TranslateAnimation a = new TranslateAnimation(0, 0,
							downItemsMoveDistance, 0);
					a.setDuration(getDelAnimationDuration());
					child.setAnimation(a);
				}
			}
		...
		// // 执行动画
		// private int doAnimation(int childrenTop, int childrenBottom,int
		// baseline, boolean animationUp, View child)
		// {
		// Point oldTop = (Point) child.getTag(ANIMATION_TAG);
		// if(oldTop == null)
		// {
		// if(animationUp)
		// {
		// TranslateAnimation a = new TranslateAnimation(0, 0, baseline -
		// child.getTop(), 0);
		// a.setDuration(getDelAnimationDuration());
		// child.setAnimation(a);
		// baseline+= child.getHeight() + mDividerHeight;
		// }
		// else
		// {
		// TranslateAnimation a = new TranslateAnimation(0, 0, baseline -
		...
		// if(oldTop.x != top)
		// {
		// TranslateAnimation a = new TranslateAnimation(0, 0, oldTop.x - top,
		// 0);
		// a.setDuration(getDelAnimationDuration());
		// child.setAnimation(a);
		// }
		// if(!animationUp && oldTop.x - mDividerHeight < childrenTop)
		// {
		// baseline = oldTop.x - mDividerHeight;
		// }
		// if(animationUp && oldTop.y + mDividerHeight > childrenBottom)
		// {
		// baseline = oldTop.y + mDividerHeight;
		// // baseline = childrenTop;
		// }
		// }
		// child.setTag(ANIMATION_TAG, null);
		// return baseline;
		...
		private int calcNewPositionAdd(int oldPosition, int first, int last,
				int firstPosition, int lastPosition) {
			if (oldPosition > lastPosition || oldPosition < firstPosition) {
				return INVALID_POSITION;
			}
			if (oldPosition >= first) {
				return oldPosition + (last - first + 1);
			}
			return oldPosition;
		}

		private int calcNewPosition(int oldPosition, int first, int last,
				int firstPosition, int lastPosition) {
			if (oldPosition > lastPosition || oldPosition < firstPosition) {
				return INVALID_POSITION;
			}
	...
	// this.mContentBackgroundPaint = paint;
	// this.mOverScrollHeaderShadow =
	// getResources().getDrawable(R.drawable.listview_header_shadow);
	// }
	// }

	public void setContentBackground(int resId) {
		setContentBackground(getContext().getResources().getDrawable(resId));
	}

	public void setContentBackground(Drawable drawable) {
		if (drawable == null) {
			mContentBackgroundDrawable = null;
			this.mOverScrollHeaderShadow = null;
		} else {
			mContentBackgroundDrawable = drawable;
			this.mOverScrollHeaderShadow = getResources().getDrawable(
					R.drawable.listview_header_shadow);
		}
	}

	public void showOverScrollHeader() {
		// smoothScrollBy会判断当listview第一个元素在顶端时offset不能小于0
		// mListView.smoothScrollBy(-mOverScrollHeader.getHeight()-1, 200);
		scrollTo(0, -getOverScrollHeight() - 1);
		if (mOverScrollViewListener != null) {
			boolean shouldShowHeaderView = mOverScrollViewListener
					.onViewCompleteVisableAndReleased(
							-getOverScrollHeight() - 1,
							mOverscrollHeaderView.getChildAt(0), this);
	...
	public boolean isOverscrollHeadVisiable() {
		return mScrollY < 0 && mOverscrollHeaderView != null;
	}
	...
	// {
	// SkinEngine.invalidateAll(header.view);
	// }
	//
	// for (FixedViewInfo footer : mFooterViewInfos)
	// {
	// SkinEngine.invalidateAll(footer.view);
	// }
	// }

	public int getOverScrollHeadState() {
		return mOverscrollHeadState;
	}
}
