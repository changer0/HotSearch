	...
	class ListDataSetObservable extends DataSetObservable {
		public void notifyRowInserted(int first, int last) {
			synchronized (mObservers) {
				...
				// anything, including
				// removing itself from {@link mObservers} - and that could
				// cause problems if
				// an iterator is used on the ArrayList {@link mObservers}.
				// to avoid such problems, just march thru the list in the
				// reverse order.
				for (int i = mObservers.size() - 1; i >= 0; i--) {
					DataSetObserver dataSetObserver = mObservers.get(i);
					if (dataSetObserver instanceof ListDataSetListener) {
						((ListDataSetListener) dataSetObserver).onRowInserted(
								first, last);
					} else {
		...
		public void notifyRowDeleted(int first, int last) {
			synchronized (mObservers) {
				...
				for (int i = mObservers.size() - 1; i >= 0; i--) {
					DataSetObserver dataSetObserver = mObservers.get(i);
					if (dataSetObserver instanceof ListDataSetListener) {
						((ListDataSetListener) dataSetObserver).onRowDeleted(
								first, last);
					} else {
						dataSetObserver.onChanged();
					}
		...
		public void notifyRowDeleted(int... rows) {
			synchronized (mObservers) {
				...
				for (int i = mObservers.size() - 1; i >= 0; i--) {
					DataSetObserver dataSetObserver = mObservers.get(i);
					if (dataSetObserver instanceof ListDataSetListener) {
						((ListDataSetListener) dataSetObserver)
								.onRowDeleted(rows);
					} else {
						dataSetObserver.onChanged();
					}
		...
		public void notifyRowUpdated(int first, int last) {
			...
			for (int i = mObservers.size() - 1; i >= 0; i--) {
				DataSetObserver dataSetObserver = mObservers.get(i);
				if (dataSetObserver instanceof ListDataSetListener) {
					((ListDataSetListener) dataSetObserver).onRowUpdated(first,
							last);
				} else {
					dataSetObserver.onChanged();
				}
