...
package com.qq.reader.widget.stickygridview;
...
public class StickyGridHeadersGridView extends GridView implements OnScrollListener,
    ...
    private static MotionEvent.PointerProperties[] getPointerProperties(MotionEvent e) {
        int n = e.getPointerCount();
    ...
    public View getHeaderAt(int position) {
        ...
        try {
            ...
            Log.printErrStackTrace("StickyGridHeadersGridView", e, null, null);
        }
        return null;
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mHeaderChildBeingPressed) {
            ...
            if (tempHeader != null) {
                ...
                tempHeader.postDelayed(() -> invalidate(0, headerHolder.getTop(), getWidth(), headerHolder.getTop()+headerHolder.getHeight()), ViewConfiguration.getPressedStateDuration());
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        StickyGridHeadersBaseAdapter baseAdapter = (StickyGridHeadersBaseAdapter)adapter;
    ...
    private MotionEvent transformEvent(MotionEvent e, int headerPosition) {
        ...
        int source = MotionEventCompat.getSource(e);//e.getSource();
        int flags = edgeFlags;
        float x = e.getX();
        float y = e.getY();
        ...
        if (Build.VERSION.SDK_INT >= 9) {
            ...
            if (Build.VERSION.SDK_INT < 14) {
                n = MotionEvent.obtain(downTime, eventTime, action,
                        pointerCount, pointerIds, pointerCoords, metaState, xPrecision,
                        yPrecision, deviceId, edgeFlags, source, flags);
            } else {
                MotionEvent.PointerProperties[] properties = getPointerProperties(e);
                int btnState = e.getButtonState();
                n = MotionEvent.obtain(downTime, eventTime, action,
                        pointerCount, properties, pointerCoords, metaState, btnState, xPrecision,
                        yPrecision, deviceId, edgeFlags, source, flags);
            }

        } else if(Build.VERSION.SDK_INT >= 14){
            n = MotionEvent.obtain(downTime, eventTime, action,
                    pointerCount, x, y, size,metaState, xPrecision, yPrecision, deviceId, edgeFlags);
        }
        return n;
    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (int i = 0; i < headerPositions.size(); i++) {
            ...
            boolean headerIsStickied = false;
            try {
                headerIsStickied = ((StickyGridHeadersBaseAdapterWrapper.HeaderFillerView)frame).getHeaderId() == mCurrentHeaderId
