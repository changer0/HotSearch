...
public class BaseViewHolder extends RecyclerView.ViewHolder {
    ...
    public BaseViewHolder(final View view) {
        ...
        this.itemChildLongClickViewIds = new LinkedHashSet<Integer>();
    ...
    public <T extends View> T getView(@IdRes int viewId) {
        return getView(viewId, 0);
    }

    @SuppressWarnings("unchecked")
    public <T extends View> T getView(@IdRes int viewId, int parentViewId) {
        String generateId = viewId + "-" + parentViewId;
        View view = views.get(generateId);
        if (view == null) {
            View parentView;

            //表示同时引用多个布局
            if (parentViewId > 0) {
                parentView = mParentViews.get(parentViewId);
                if (parentView == null) {
                    parentView = itemView.findViewById(parentViewId);
                    mParentViews.put(parentViewId, parentView);
                }
            } else {
                parentView = itemView;
            }
            view = parentView.findViewById(viewId);
            views.put(generateId, view);
        }
        return (T) view;
    }
