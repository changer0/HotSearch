...
import com.qq.reader.widget.recyclerview.animation.AlphaInAnimation;
import com.qq.reader.widget.recyclerview.animation.BaseAnimation;
import com.qq.reader.widget.recyclerview.animation.ScaleInAnimation;
import com.qq.reader.widget.recyclerview.animation.SlideInBottomAnimation;
import com.qq.reader.widget.recyclerview.animation.SlideInLeftAnimation;
import com.qq.reader.widget.recyclerview.animation.SlideInRightAnimation;
import com.qq.reader.widget.recyclerview.entity.IExpandable;
import com.qq.reader.widget.recyclerview.listener.OnItemChildClickListener;
import com.qq.reader.widget.recyclerview.listener.OnItemChildLongClickListener;
import com.qq.reader.widget.recyclerview.loadmore.LoadMoreView;
import com.qq.reader.widget.recyclerview.util.MultiTypeDelegate;
...
public abstract class BaseQuickAdapter<T, K extends BaseViewHolder> extends RecyclerView.Adapter<K> {
    ...
    private LoadMoreView mLoadMoreView = new ReaderLoadMoreView();
     ...
     * 防止每次都要重新创建布局
     */
    private int mCurrentEmptyLayoutId = -1;
    ...
    private SparseArray<BaseCertainItemView> mCertainItemViews = new SparseArray<>();
     ...
     */
    public void setReaderLoadMoreView(LoadMoreView loadingView) {
    ...
    public void addData(@IntRange(from = 0) int position, @NonNull T data) {
        ...
        notifyItemInserted(position + getHeaderLayoutCount() + getCertainItemNotifyIndexCount(position));
    ...
    public void addData(@NonNull T data) {
        mData.add(data);
        notifyItemInserted(mData.size() + getHeaderLayoutCount() + getCertainItemNotifyIndexCount(mData.size()));
        compatibilityDataSizeChanged(1);
    }
    ...
    public void remove(@IntRange(from = 0) int position) {
        ...
        int internalPosition = position + getHeaderLayoutCount() + getCertainItemNotifyIndexCount(position);
        notifyItemRemoved(internalPosition);
        compatibilityDataSizeChanged(0);
        notifyItemRangeChanged(internalPosition, mData.size() - internalPosition + getCertainItemNotifyIndexCount(position));
    }
    ...
    public void setData(@IntRange(from = 0) int index, @NonNull T data) {
        ...
        notifyItemChanged(index + getHeaderLayoutCount() + getCertainItemNotifyIndexCount(index));
    }
    ...
    public void addData(@IntRange(from = 0) int position, @NonNull Collection<? extends T> newData) {
        ...
        notifyItemRangeInserted(position + getHeaderLayoutCount() + getCertainItemNotifyIndexCount(position), newData.size());
    ...
    public void addData(@NonNull Collection<? extends T> newData) {
        ...
        notifyItemRangeInserted(mData.size() - newData.size() + getHeaderLayoutCount() + getCertainItemNotifyIndexCount(mData.size() - newData.size()), newData.size());
        compatibilityDataSizeChanged(newData.size());
    }
    ...
    /**
     * 用于插入删除，计算mData开始刷新位置
     */
    public int getCertainItemNotifyIndexCount(int position) {
        int indexCount = 0;
        for (int i = 0; i < mCertainItemViews.size(); i++) {
            if (mCertainItemViews.keyAt(i) >= position) {
                indexCount++;
            }
        }
    ...
    public int getCertainItemFirstIndexByPosition(int position) {
        for (int i = 0; i < mCertainItemViews.size(); i++) {
            if (mCertainItemViews.valueAt(i).getCertainItemIndex() >= position) {
                return i;
            }
        }
        return -1;
    ...
    /**
     * 为了保证mData点击position不受影响，
     * 这里需要加上certainItem 的index
     */
    public int getCertainItemClickIndexCount(int position) {
        int indexCount = 0;
        for (int i = 0; i < mCertainItemViews.size(); i++) {
            if (mCertainItemViews.keyAt(i) <= position) {
                indexCount++;
    ...
    public int getCertainItemViewCount() {
        if (mCertainItemViews == null || mCertainItemViews.size() == 0) {
            return 0;
        }

        return mCertainItemViews.size();
    }
    ...
    public int getItemCount() {
        ...
        if (getEmptyViewCount() == 1) {
            ...
            count = getHeaderLayoutCount() + mData.size() + getFooterLayoutCount() + getLoadMoreViewCount() + getCertainItemViewCount();
        }
        return count;
    }
    ...
    public int getContentItemCount() {
        return getCount() + getCertainItemCount();
    }

    public int getCertainItemCount() {
        if (mCertainItemViews == null || mCertainItemViews.size() == 0) {
            return 0;
        }
        return mCertainItemViews.size();
    ...
    protected int getCertainItemViewType(int position) {
        if (mCertainItemViews.indexOfKey(position) >= 0) {
            return mCertainItemViews.get(position).getLayoutId();
        } else {
            return -1;
        }
    }
    ...
    public int getItemViewType(int position) {
        ...
        if (position < numHeaders) {
            ...
            int adapterCount = mData.size() + mCertainItemViews.size();

//            if (mData.size() == 0) {
//                adapterCount = 0;
//                //当size等于0时，只绘制插入第0个位置的item
//                if (adjPosition == 0 && mCertainItemViews.get(0) != null) {
//                    return getCertainItemViewType(0);
//                }
//            }

            if (adjPosition < adapterCount) {
                //分三种情况，    `
                //1、插入位置均大于mData.size()，依次排在item后面
                //2、插入位置均小于mData.size()，依次排在item前面
                //3、mData.size()位于插入位置中间的，
                if (mCertainItemViews.indexOfKey(adjPosition) >= 0) {
                    if (mCertainItemViews.get(adjPosition) != null) {
                        return mCertainItemViews.get(adjPosition).getLayoutId();
                    } else {
                        return getDefItemViewType(adjPosition);
                    }
                } else {
                    if (adjPosition >= mData.size()) {
                        int key = adjPosition - mData.size();
                        if (key < mCertainItemViews.size()
                                && mCertainItemViews.keyAt(key) > mData.size()
                                && mCertainItemViews.valueAt(key) != null) {
                            return mCertainItemViews.valueAt(key).getLayoutId();
                        } else {
                            return getDefItemViewType(adjPosition);
                        }
                    } else {
    ...
    public K onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case FOOTER_VIEW:
                ...
                break;
            default:
                if (hasContainsCertainViewLayoutId(viewType)) {
    ...
    public void onBindViewHolder(K holder, int position) {
        ...
        int convertPosition = position - getHeaderLayoutCount();
        ...
        switch (viewType) {
            ...
            case FOOTER_VIEW:
                ...
                break;
            default:
                if (mCertainItemViews.indexOfKey(convertPosition) < 0 && convertPosition < mData.size() + getCertainItemClickIndexCount(convertPosition)) {
                    convert(holder, convertPosition - getCertainItemClickIndexCount(convertPosition));
                } else {
                    if (convertPosition >= mData.size()) {
                        int key = getCertainItemFirstIndexByPosition(convertPosition);
                        if (key < mCertainItemViews.size()
                                && mCertainItemViews.valueAt(key) != null) {
                            mCertainItemViews.valueAt(key).onBindViewHolder(holder);
                        }
                    } else {
                        if (mCertainItemViews.indexOfKey(convertPosition) >= 0) {
                            mCertainItemViews.get(convertPosition).onBindViewHolder(holder);
                        }
                    }
                }
                break;
    ...
    private void bindViewClickListener(final BaseViewHolder baseViewHolder) {
        ...
        if (getOnItemClickListener() != null) {
            view.setOnClickListener(v -> {
                int convertPosition = baseViewHolder.getLayoutPosition() - getHeaderLayoutCount();

                Log.d(TAG, "convertPosition=" + convertPosition + " getCertainItemClickIndexCount=" + getCertainItemClickIndexCount(convertPosition));

                setOnItemClick(v, convertPosition - getCertainItemClickIndexCount(convertPosition));
            });
        }
        if (getOnItemLongClickListener() != null) {
            ...
            view.setOnLongClickListener(v -> {
                int convertPosition = baseViewHolder.getLayoutPosition() - getHeaderLayoutCount();
                return setOnItemLongClick(v, convertPosition - getCertainItemClickIndexCount(convertPosition));
            });
        }
    }

    public int getCertainViewByLayoutId(int viewType) {
        if (mCertainItemViews == null || mCertainItemViews.size() == 0) {
            return -1;
        }

        for (int i = 0; i < mCertainItemViews.size(); i++) {
            if (mCertainItemViews.valueAt(i).getLayoutId() == viewType) {
                return mCertainItemViews.valueAt(i).getLayoutId();
            }
        }
    ...
    private boolean hasContainsCertainViewLayoutId(int viewType) {
        if (mCertainItemViews == null || mCertainItemViews.size() == 0) {
            return false;
        }
    ...
    public void addCertainItemView(int index, BaseCertainItemView insertView) {
        if (mCertainItemViews != null) {
            if (mCertainItemViews.indexOfKey(index) < 0) {
                mCertainItemViews.put(index, insertView);
                notifyDataSetChanged();
            }
        }
    }

    public void removeCertainItemView(int index) {
        if (mCertainItemViews != null) {
            if (mCertainItemViews.indexOfKey(index) >= 0) {
                mCertainItemViews.remove(index);
                notifyDataSetChanged();
            }
        }
    }

    public void removeCertainItemView(BaseCertainItemView certainView) {
        if (mCertainItemViews != null) {
            if (mCertainItemViews.indexOfValue(certainView) >= 0) {
                mCertainItemViews.removeAt(mCertainItemViews.indexOfValue(certainView));
                notifyDataSetChanged();
            }
        }
    }
    ...
    public int getFooterViewPosition() {
        ...
        if (getEmptyViewCount() == 1) {
            ...
            return getHeaderLayoutCount() + getContentItemCount();
        }
        return -1;
    }
     ...
     * 以下四个方法均可以在子类中重写
     * 但是不会涉及任何View相关的操作
     * 目前的实现逻辑以下四个View在展示过程中不需要重新bind
     * 所以可以作为辅助方法按需重写（例如埋点）
     * 表示当前屏幕展示到这个item
     */
    protected void attachToFooterView() {
    }

    ;

    protected void attachToHeaderView() {
    }

    ;

    protected void attachToEmptyView() {
    }
