...
public abstract class BaseViewBindItem<T extends BaseDataBean, Holder extends RecyclerView.ViewHolder> {
    ...
    public void attachView(final Holder holder) throws Exception {
        ...
        if (activity == null || !bindView(holder, activity)) {
            ...
            if (BuildConfig.DEBUG) {
                Toast.makeText(DataProviderConfig.getApplication(), "ViewBindItem 加载未加载成功： " + this, Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private Activity getActivity() {
        ...
        activity = ProviderUtility.getActivity(context);
        if (activity == null) {
            if (BuildConfig.DEBUG) {
                String errorTip = "请使用 Activity 的 Context：" + getClass().getName();
                Toast.makeText(DataProviderConfig.getApplication(), errorTip, Toast.LENGTH_SHORT).show();
